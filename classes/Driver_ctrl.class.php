...
class Driver_ctrl
{
    public function register()
    {
        ...
        if (
            ...
            && isset($req->password) && (isset($req->national_id))
        ) {
            ...
            try {
                ...
                if (intval($id)) {
                    ...
                    $db->update();
                    $_SESSION['msg'][] = "Congratulations you have successfully registered as a driver";
                    $pdo->commit();
                    echo js_alert(msg_ssn(return: true));
                    return true;
                } else {
    ...
    function login()
    {
        if (isset($_POST['email']) && isset($_POST['password'])) {
            ...
            $db = new Dbobjects;
            $db->tableName = 'pk_user';
            $user_email = $db->filter(['email' => $req->email, 'password' => md5($req->password),'is_driver'=>1]);
            if (count($user_email) == 1) {
                $user = $user_email[0];
            } else {
                $user_username = $db->filter(['username' => $req->email, 'password' => md5($req->password),'is_driver'=>1]);
